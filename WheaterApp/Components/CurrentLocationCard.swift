...
struct CurrentLocationCard: View {
    ...
    var body: some View {
        ...
        .onChange(of: latitude){ oldValue, newValue in
            if newValue != 0 {
                fetchWeather(lat: newValue, lon: longitude)
            }
        }
        .onAppear {
            if latitude != 0 {
    ...
    func fetchWeather(lat: Double, lon: Double){
        Task{
            do {
                weather = try await ApiNetwork().getWeatherByCoordinate(latitude: latitude, longitude: longitude)
            } catch {
                print("Error \(error)")
            }
        }
...
#Preview {
    CurrentLocationCard(location: GPSManager(), latitude: 0, longitude: 0)
}
